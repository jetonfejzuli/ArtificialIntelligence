...
if __name__ == '__main__':
    ...
    classifier3 = RandomForestClassifier(4, criterion='entropy', random_state=0)
    classifier4 = MLPClassifier(10, activation='relu', learning_rate_init=0.001, random_state=0)
    ...
    for x, y in zip(test_x, test_y):
        pred1 = classifier1.predict([x])[0]
        pred2 = classifier2.predict([x])[0]
        pred3 = classifier3.predict([x])[0]
        pred4 = classifier4.predict([x])[0]
        if pred1 == y:
            acc1 += 1
        if pred2 == y:
            acc2 += 1
    ...
    acc3 = acc3 / len(test_x)
    acc2 = acc2 / len(test_x)
    acc4 = acc4 / len(test_x)

    accs = [acc1, acc2, acc3, acc4]
    highest = accs.index(max(accs))

    if highest == 0:
        print('The highest accuracy was achieved by the classifier Naive Bayes')
    elif highest == 1:
        print('The highest accuracy was achieved by the classifier Decision Tree')
    elif highest == 2:
    ...
    models = [classifier1, classifier2, classifier3, classifier4]
    ...
    for x, y in zip(test_x, test_y):
        ...
        for i, model in enumerate(models):
            ...
            if prediction == 0:
                votes_0 += 2 if i == highest  else 1
            else:
                votes_1 += 2 if i == highest else 1

        final_prediction = 0 if votes_0 > votes_1 else 1

        if final_prediction == 1 and final_prediction == y:
            tp += 1
        elif final_prediction == 0 and final_prediction != y:
            fn += 1

    recall = tp / (tp + fn) if tp + fn != 0 else 0
    print(f'The recall of the ensemble classifier is {recall}')
