...
if __name__ == '__main__':
    ...
    train_x = [[float(v) for i, v in enumerate(row[:-1]) if i != column_index] for row in train_set]
    train_y = [row[-1] for row in train_set]

    test_set = dataset[split_index:]
    test_x = [[float(v) for i, v in enumerate(row[:-1]) if i != column_index] for row in test_set]
    ...
    input_record = [v for i, v in enumerate(input_record) if i != column_index]
    predition = classifier.predict([input_record])[0]
    print(predition)
