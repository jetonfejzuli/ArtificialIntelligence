...
if __name__ == '__main__':
    ...
    classifier1 = MLPClassifier(neurons, activation='relu', learning_rate_init=l_rate, max_iter=20, random_state=0)
    classifier1.fit(minMax.transform(train_x), train_y)

    acc1 = 0
    for x, y in zip(train_x, train_y):
        pred = classifier1.predict(minMax.transform([x]))[0]
        if pred == y:
            acc1 += 1

    acc1 = acc1 / len(train_set)
    ...
    if acc1 > acc2 * 1.15:
        ...
        input_record = input_record[:col] + input_record[col + 1:]
        minMax = MinMaxScaler((-1, 1))
        minMax.fit(train_x)
        classifier1.fit(minMax.transform(train_x), train_y)
        predicted_class = classifier1.predict(minMax.transform([input_record]))[0]
    else:
