...
if __name__ == '__main__':
    ...
    train_y = [1 if i >= 5 else 0 for i in train_y]

    test_x = [row[:-1] for row in test_set]
    test_y = [row[-1] for row in test_set]
    test_y = [1 if i >= 5 else 0 for i in test_y]

    classifier = DecisionTreeClassifier(criterion='gini', random_state=0)
    classifier.fit(train_x, train_y)

    features_importances = list(classifier.feature_importances_)
    least_index = features_importances.index(min(features_importances))
    ...
    classifier3.fit(standard.transform(train_x), train_y)
    ...
    print(f'Accuracy with StandardScaler: {acc2}')
    print(f'Accuracy with MinMaxScaler: {acc1}')
