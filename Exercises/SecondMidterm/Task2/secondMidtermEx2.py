...
if __name__ == '__main__':
    ...
    for x, y_true in zip(test_x, test_y):
        predict = classifier.predict([x])[0]
        if predict == y_true:
            correct += 1
    accurracy = correct / len(test_set)
    print(accurracy)
    input_record = input().strip().split()
    input_record = [[float(v) for v in input_record]]
    predicted_class = classifier.predict(input_record)[0]
    print(predicted_class)
    class_predict = classifier.predict_proba(input_record)
    print(class_predict)
